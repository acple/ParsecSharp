...
namespace ParsecSharp.Examples
{
    ...
    public static class CsvParser
    {
        ...
        // DQUOTE = %x22 ; == '"'
        private static readonly Parser<char, char> DoubleQuote =
            Char('"');
        ...
        private static readonly Parser<char, char> TextChar =
            Any().Except(Comma, DoubleQuote, ControlChar());

        // escaped = DQUOTE *( TEXTDATA / COMMA / CR / LF / 2DQUOTE ) DQUOTE
        // 二重引用符で囲まれたフィールド値
        // 改行文字は ( LF / CRLF ) のどちらかのみに対応
        private static readonly Parser<char, string> EscapedField =
            Many(Choice(TextChar, Comma, EndOfLine(), DoubleQuote.Right(DoubleQuote)))
                .Between(DoubleQuote)
                .AsString();

        // non-escaped = *TEXTDATA
        private static readonly Parser<char, string> NonEscapedField =
            Many(TextChar).AsString();

        // field = ( escaped / non-escaped )
        private static readonly Parser<char, string> Field =
            EscapedField | NonEscapedField;

        // record = field *( COMMA field )
        private static readonly Parser<char, string[]> Record =
        ...
        private static readonly Parser<char, IEnumerable<string[]>> Csv =
