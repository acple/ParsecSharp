...
namespace ParsecSharp
{
    public static class ParsecState
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ParsecStateStream<TToken, LinearPosition<TToken>> Tokenize<TInput, TState, TToken>(TState source, Parser<TInput, TToken> parser)
            where TState : IParsecState<TInput, TState>
            => Tokenize(source, parser, LinearPosition<TToken>.Initial);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ParsecStateStream<TToken, TPosition> Tokenize<TInput, TState, TToken, TPosition>(TState source, Parser<TInput, TToken> parser, TPosition position)
            where TState : IParsecState<TInput, TState>
            where TPosition : IPosition<TToken, TPosition>
            => Tokenize(source.InnerResource, parser.ParsePartially(source), parser, position);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ParsecStateStream<TToken, TPosition> Tokenize<TInput, TToken, TPosition>(IDisposable? resource, SuspendedResult<TInput, TToken> state, Parser<TInput, TToken> parser, TPosition position)
            where TPosition : IPosition<TToken, TPosition>
