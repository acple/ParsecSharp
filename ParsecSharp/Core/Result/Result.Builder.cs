...
namespace ParsecSharp.Internal
{
    public static class Result
    {
        ...
        public static Result<TToken, T> Failure<TToken, TState, T>(TState state)
            where TState : IParsecState<TToken, TState>
            => new ParseError<TToken, TState, T>(state);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Result<TToken, T> Failure<TToken, TState, T>(Exception exception, TState state)
            where TState : IParsecState<TToken, TState>
            => new FailureWithException<TToken, TState, T>(exception, state);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Result<TToken, T> Failure<TToken, TState, T>(string message, TState state)
            where TState : IParsecState<TToken, TState>
