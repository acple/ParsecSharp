...
namespace ParsecSharp
{
    public static partial class Parser
    {
        ...
        private static IParser<TToken, IEnumerable<T>> ManyRec<TToken, T>(IParser<TToken, T> parser, IEnumerable<T> result)
            => parser.Next(x => ManyRec(parser, result.Append(x)), result);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static IParser<TToken, IEnumerable<T>> ManyTillRec<TToken, T, TIgnore>(IParser<TToken, T> parser, IParser<TToken, TIgnore> terminator, IEnumerable<T> result)
