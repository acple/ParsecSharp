...
namespace Parsec
{
    public static partial class Parser
    {
        ...
        public static Parser<TToken, T> Rec<TToken, T>(Parser<TToken, T> parser, Func<T, Parser<TToken, T>> rest)
            => parser.Bind(x => Rec_(parser, rest, x));

        private static Parser<TToken, T> Rec_<TToken, T>(Parser<TToken, T> parser, Func<T, Parser<TToken, T>> rest, T value)
            => Try(rest(value).Bind(x => Rec_(parser, rest, x)), value);
